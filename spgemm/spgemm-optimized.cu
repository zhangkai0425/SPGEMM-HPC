...
const char* version_name = "optimized version (Best==V1) : dividing only by rows";\
...
void preprocess(dist_matrix_t *matA, dist_matrix_t *matB) {
    ...
    dim3 block,grid;
    // Divide the threads by row of A
    block.x = 1;
    block.y = 1;
...
__global__ void spgemm_kernel  (const index_t* d_r_pos_A,const index_t* d_c_idx_A,const data_t* d_val_A,
    ...
    // A[i][j]
    for (int ptr_a = ptr_a_begin; ptr_a < ptr_a_end; ++ptr_a) {
        sm_idx_A = d_c_idx_A[ptr_a];
        sm_val_A = d_val_A[ptr_a];
        // C[i][:] += A[i][j] * B[j][:]
        index_t col_A = sm_idx_A;
        index_t ptr_b_begin = d_r_pos_B[col_A];
        index_t ptr_b_end = d_r_pos_B[col_A + 1];
        for(int col_id_idx = ptr_b_begin; col_id_idx < ptr_b_end; ++col_id_idx) {
            index_t col_B = d_c_idx_B[col_id_idx];
            data_t val = sm_val_A * d_val_B[col_id_idx];
